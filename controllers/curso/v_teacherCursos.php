...
if ($filter == "todos") {
    $cursos = $db->query("SELECT * FROM v_cursosMaestro WHERE ID_maestro = ?", [
        $_SESSION['user']['id_rol']
    ])->get();
} else if ($filter == "activos") {
    $cursos = $db->query("SELECT * FROM v_cursosMaestro WHERE estatus = 1 AND ID_maestro = ?", [
        $_SESSION['user']['id_rol']
    ])->get();
} else if ($filter == "inactivos") {
    $cursos = $db->query("SELECT * FROM v_cursosMaestro WHERE estatus = 0 AND ID_maestro = ?", [
        $_SESSION['user']['id_rol']
    ])->get();
} 
else {
    ...
    if($complexfilter[0] == "cat"){
        $cursos = $db->query("SELECT * FROM v_cursosMaestro WHERE ID_categoria = ? AND ID_maestro = ?", [
            $complexfilter[1],
            $_SESSION['user']['id_rol']
            ])->get();
        ...
        if (DateTime::createFromFormat('Y-m-d', $startDate) && DateTime::createFromFormat('Y-m-d', $endDate)) {
            $cursos = $db->query(
                "SELECT * FROM v_cursosMaestro WHERE f_creacion BETWEEN ? AND ? AND ID_maestro = ?",
